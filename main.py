    ...
    while True:
        ...
        if not next_interval:
            ...
            if LastRun == 0:
                ...
                last_run_dt = dt.datetime.utcfromtimestamp(LastRun).replace(tzinfo=dt.timezone.utc)
        ...
        if not firstRun:
            ...
            trade_log[dt.datetime.strftime(next_interval+dt.timedelta(seconds=1),"%Y-%m-%d %H:%M:%S")] = {"close":None,"prediction":result,"predicted_direction_from_current":direction,"previous_close":last_price,"correct_prediction":None,"took_trade":took_trade}
        ...
        print("Loaded new DataFrame and updating with JSON new records.")
        df = pd.read_json(f"JSON\\{ticker_file}_{interval}_price_log.json", orient="index", convert_dates=False)
        df.index.name = "date"
